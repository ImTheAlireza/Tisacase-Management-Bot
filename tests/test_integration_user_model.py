...
def _get_pl_row(prefix: str) -> dict | None:
...
def _insert_pl(db_conn, prefix='TS', is_active=True) -> int:
...
class TestGetStats:
    ...
    def test_stats_counts_correctly(self, db_conn):
        ...
        cursor = db_conn.cursor()
        # Insert designs with different statuses
        for code, status in [
            ('TS001', 'pending'),
            ('TS002', 'pending'),
            ('TS003', 'approved'),
