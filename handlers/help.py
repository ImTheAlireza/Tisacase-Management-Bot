...
HELP_CATEGORIES = {
    'main': {
        ...
        'buttons': [
            ...
            [InlineKeyboardButton("💬 تنظیم گروه‌ها", callback_data="help_groups")],
            [InlineKeyboardButton("🔢 مدیریت کدها", callback_data="help_codes")],
            [InlineKeyboardButton("🗑 حذف و پاکسازی", callback_data="help_cleanup")],
            [InlineKeyboardButton("⚙️ سیستم", callback_data="help_system")],
        ]
    },
    'users': {
        'text': (
            ...
            "📝 نکته: غیرفعال کردن کاربر باعث حذف او نمی‌شود.\n"
    ...
    'lines': {
        'text': (
            ...
            "مثال: /enableline MG\n\n"
            "🗑 حذف کامل خط:\n"
            "دستور: /deleteline {prefix}\n"
            "مثال: /deleteline MG\n"
            "فقط خطوطی قابل حذف هستند که هیچ طرح تایید شده‌ای نداشته باشند.\n"
            "اگر خط طرح تایید شده داشته باشد، فقط غیرفعال کردن امکان‌پذیر است.\n\n"
            "💬 تنظیم گروه‌های هر خط:\n"
            "از دکمه 💬 تنظیم گروه‌ها در منوی اصلی استفاده کنید"
        ),
        'back': 'main'
    },
    'groups': {
        'text': (
            ...
            "Chat ID معمولا با ‎-100 شروع می‌شود.\n"
            "مثال: ‎-1001234567890\n\n"
    ...
    'codes': {
        'text': (
            ...
            "دستور: /lockcode {code}\n"
            "مثال: /lockcode TS050\n"
            "کد قفل شده در کدگذاری بعدی رد می‌شود\n\n"
    ...
    'cleanup': {
        'text': (
            ...
            "\n\n"
            "💀 حذف کامل یک طرح:\n"
            "دستور: /deletedesign {code}\n"
            "مثال: /deletedesign TS001\n\n"
            "این دستور بسته به وضعیت طرح:\n"
            "• pending: پیام‌های ناظران از PV آن‌ها حذف می‌شود\n"
            "• approved: فایل‌ها از گروه‌های محصولات و چاپ حذف می‌شود\n"
            "• rejected: فقط رکورد دیتابیس حذف می‌شود\n"
            "• در همه حالت‌ها: کد آزاد شده و قابل استفاده مجدد است\n\n"
            "⚠️ این عملیات غیرقابل بازگشت است!\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n\n"
            "🧹 پاکسازی کدهای孤رها:\n"
            "دستور: /cleanup\n\n"
            "طرح‌های pending قدیمی‌تر از ۲۴ ساعت که فایلی ندارند\n"
            "به صورت خودکار پیدا و حذف می‌شوند.\n"
            "این کدها معمولا زمانی ایجاد می‌شوند که:\n"
            "• طراح در حین آپلود، ثبت جدید شروع کرده\n"
            "• طراح بدون ارسال فایل، پروسه را رها کرده\n"
            "• timeout رخ داده و session بسته شده\n\n"
            "━━━━━━━━━━━━━━━━━━━━\n\n"
            "📊 مشاهده طرح‌های در انتظار:\n"
            "دستور: /pendingdesigns\n"
            "لیست همه طرح‌های pending با کد، طراح و تعداد فایل‌ها"
        ),
        'back': 'main'
    },
    'system': {
        'text': (
            ...
            "📡 وضعیت ربات:\n"
            "دکمه: 📡 وضعیت\n"
            "نمایش وضعیت فعلی process ربات در supervisor\n\n"
            "🚨 خطاهای سیستمی:\n"
...
def _build_markup(category_key):
    ...
    if 'back' in category:
        buttons.append([InlineKeyboardButton("🔙 بازگشت", callback_data=f"help_{category['back']}")])
...
async def help_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        HELP_CATEGORIES[category_key]['text'],
